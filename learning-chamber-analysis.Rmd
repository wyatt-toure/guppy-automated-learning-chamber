...
# Reading in data
full.data = read.csv('data/learning-chamber-data.csv')
...
First we want to make sure the data is properly formatted. We will use the `describe_all_cat` function from the package `tidyext` to do so. This will summarise all of our categorical variables. Note I filtered out ID for this table. 
...
We see that some of the values are below 175 so these will have to be checked. 
...
time.plot = ggplot(full.data,
  ...
  geom_hline(yintercept = -5.89, linetype = 'dashed', alpha = 0.4)
...
latency.plot = ggplot(full.data,
  ...
  scale_x_discrete(labels = testing.data.x.axis.labels) +
  stat_summary(geom = 'errorbar', fun.data = 'mean_se', position = position_dodge(width=0), width = 0.1) + 
  geom_hline(yintercept = -8.07, linetype = 'dashed', alpha = 0.5)
...
There is a significant increase in the proportion of visits to the rewarded feeder after the light turns on (p = 0.039). Guppies go from making 44.4% of their visits to the rewarded feeder before the light shines to making 56.6% of their visits being to the rewarded feeder after the light shines, an increase of 12.2%. 
...
visits.plot = ggplot(full.data,
  ...
  scale_x_discrete(labels = testing.data.x.axis.labels) + 
  stat_summary(geom = 'errorbar', fun.data = 'mean_se', position = position_dodge(width=0), width = 0.1) +
  geom_hline(yintercept = 0.433, linetype = 'dashed', alpha = 0.5)
...
We see that while the effect of the light turning is only significant for the proportion of visits to the feeder, the other two preference metrics (time spent on the side of the tank where the light shone and the latency to the rewarded feeder) all have an effect size that is in the direction consistent with the hypothesis that guppies can learn a food-light association in the automated chamber.  
...
3) Guppies significantly increase their proportion of visits to the rewarded feeder after the light shines (Figure 4C)
